...
export async function createGiftRecord(userId, record) {
  const result = await query(
    `insert into gift_records (event_id, guest_name, amount, gift_item, relative_title, record_date, note)
     select $1, $3, $4, $5, $6, $7, $8
     from events
     where events.id = $1 and events.user_id = $2
     returning id, guest_name, amount, gift_item, relative_title, record_date, event_id, note, created_at, updated_at`,
    [
      record.eventId,
      userId,
      record.guestName,
