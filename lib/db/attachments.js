...
export class AttachmentStorageNotMigratedError extends Error {
  constructor() {
    super('附件存储表未迁移，请先执行 db/schema.sql 中的 event_attachments file_data 迁移')
    this.name = 'AttachmentStorageNotMigratedError'
  }
}

export function buildAttachmentDownloadHeaders(attachment) {
  const fileName = attachment.display_name || attachment.original_name
  const asciiFallback = sanitizeAttachmentFileName(fileName)
...
export async function listAttachments(userId, eventId) {
  const result = await query(
    `select event_attachments.id,
     ...
     where event_attachments.event_id = $1 and ${eventAccessCondition('$2', '$3')}
     order by event_attachments.created_at desc`,
    [eventId, userId, 'attachments:view']
  )
...
export async function createAttachment(userId, eventId, file) {
  const eventResult = await query(
    ...
    [eventId, userId, 'attachments:create']
  )
  ...
  try {
    insertResult = await query(
      `insert into event_attachments (event_id, original_name, stored_name, display_name, mime_type, size_bytes, file_data)
    ...
    if (error?.code === '42703') {
      throw new AttachmentStorageNotMigratedError()
    }

    throw error
  }

  return mapAttachmentRow(insertResult.rows[0])
}
...
export async function findAttachmentForUser(userId, attachmentId) {
  const result = await query(
    `select event_attachments.id,
            ...
            event_attachments.mime_type,
            event_attachments.size_bytes,
            event_attachments.file_data,
            event_attachments.created_at
     from event_attachments
     inner join events on events.id = event_attachments.event_id
     where event_attachments.id = $1 and ${eventAccessCondition('$2', '$3')}`,
    [attachmentId, userId, 'attachments:view']
  )

  return result.rows[0] ?? null
}

export async function findAttachmentFileForUser(userId, attachmentId) {
  let result
...
export async function deleteAttachment(userId, attachmentId) {
  const attachment = await findAttachmentForUserWithPermission(
    userId,
    attachmentId,
    'attachments:delete'
  )
...
async function findAttachmentForUserWithPermission(userId, attachmentId, permission) {
  const result = await query(
    `select event_attachments.id,
            event_attachments.event_id,
            event_attachments.original_name,
     ...
     inner join events on events.id = event_attachments.event_id
     where event_attachments.id = $1 and ${eventAccessCondition('$2', '$3')}`,
    [attachmentId, userId, permission]
  )
