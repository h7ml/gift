...
export function filterGiftRecordsBySearchQuery(records, query) {
  ...
  return records.filter((record) => {
    const matchesText =
      ...
      record.returnGiftNote?.toLowerCase().includes(normalizedText)
    ...
    const matchesGiftAmount =
      record.amount >= amountRange.min && record.amount <= amountRange.max
    const matchesReturnGiftAmount =
      typeof record.returnGiftAmount === 'number' &&
      record.returnGiftAmount >= amountRange.min &&
      record.returnGiftAmount <= amountRange.max

    return matchesGiftAmount || matchesReturnGiftAmount
  })
}
